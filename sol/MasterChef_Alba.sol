...
MasterChef_Alba.sol
Contract Name: MasterChef - Alba
Compiler Version: v0.6.12+commit.27d51765
Optimization Enabled: No with 200 runs
Other Settings: default evmVersion, None license

#############################################################
contract address : 0x1E7a94A2B33BB95d2BB9f174A213Ab6E5d6bd3fD
#############################################################
-----Decoded View---------------
Arg [0] : _alba (address): 0x98A5A21B2fa55b14030c44b4C4ACe4B16f5c6C51
Arg [1] : _devaddr (address): 0x6245e28Fe45B543c1F93e65c0997114d4Ba0F39c
Arg [2] : _feeAddress (address): 0x6245e28Fe45B543c1F93e65c0997114d4Ba0F39c
Arg [3] : _albaPerBlock (uint256): 1000000000000000000
Arg [4] : _startBlock (uint256): 645100

org : https://bscscan.com/address/0x8932a6265b01d1d4e1650feb8ac38f9d79d3957b#readContract
56    pool 0 : 0x787732f27d18495494cea3792ed7946bbcff8db2 (MASH)- 0xe9e7cea3dedca5984780bafc599bd69add087d56 (BUSD)
21004 pool 0 : 0x98A5A21B2fa55b14030c44b4C4ACe4B16f5c6C51 (ALBA)- 0xe9e7cea3dedca5984780bafc599bd69add087d56 (BUSD)

owner : Timelock (0x511Df929947Cd512Dda4A00cc87363d9baea4915)
22-10-06 
0: ALBA-BUSD LP : 500,0x83267FD68ACA1f0A11f3A89F187534c404d6Aa2c,0,0
1: ALBA-C4EI LP : 500,0x05FB813418a1F7190754502Cb3085fAa968F1b70,0,0
2: C4EI-BUSD LP : 100,0x45732739B1E47821A625aF3d8F818A1DBf6c1B65,500,0
*/
...
contract AlbaToken is BEP20('AlbaDeFi', 'ALBA') {
...
contract MasterChef is Ownable {
    ...
    // The EGG TOKEN!
    AlbaToken public alba;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public albaPerBlock;
    // Bonus muliplier for early alba makers.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _albaPerBlock,
        uint256 _startBlock
    ) public {
        alba = _alba;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        albaPerBlock = _albaPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 albaReward = multiplier.mul(albaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(albaReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 albaReward = multiplier.mul(albaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        alba.mint(devaddr, albaReward.div(10));
        alba.mint(address(this), albaReward);
        pool.accEggPerShare = pool.accEggPerShare.add(albaReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe alba transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 albaBal = alba.balanceOf(address(this));
        if (_amount > albaBal) {
            alba.transfer(_to, albaBal);
        } else {
            alba.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _albaPerBlock) public onlyOwner {
        massUpdatePools();
        albaPerBlock = _albaPerBlock;
    }
}
